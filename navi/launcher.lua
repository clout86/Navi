...
local function launchCaptain()
    Talkies.clearMessages()
    bettercap_ui.showMenu()
end

local function launchRena()
    Talkies.clearMessages()
    rena.dialog()
...
function launcher.load()
    ...
    Navigator:addButton("vasButton", "Malfina", startX, buttonY2, {buttonWidth, buttonHeight}, vasButton, function()
        print("pressed Malfina")
    end)
...
function launcher.draw()
    ...
    -- Draw cursor logic...
    if currentFocusStateIndex == 5 then
...
function launcher.keypressed(key)
    ...
    if key == 'a' then
        ...
        if buttonName then
            ...
            currentFocusStateIndex = 6 -- switch to dialog
        end
    end
    if key == 'b' then
